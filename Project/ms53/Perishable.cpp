...
namespace sdds
{
   ...
   std::ofstream& Perishable::save(std::ofstream& ofstr) const
   {
      ...
      if (*this)
      {
         ...
         Item::save(ofstr);
         ofstr << "\t";
         // Kiểm tra và ghi m_instruction nếu có.
         if (m_instruction && m_instruction[0] != '\0') {
             ofstr << m_instruction; // Ghi m_instruction nếu có.
             ofstr << "\t";
         }
         else {
             ofstr << "\t";
         ...
         // Ghi m_expiry nếu có, sau đó thêm một tab. Nếu không có m_expiry, kết thúc dòng.
         D = m_expiry;
         if (m_expiry) {
         ...
         // Nếu không có m_instruction và m_expiry, xuống dòng mới.
